  ...
  useEffect(() => {
    ...
    const fetchRepos = async () => {
      ...
      try {
        ...
        setRepos(filtered.length ? filtered : []);
      } catch (e: any) {
        console.error("Projects fetch error:", e);
        // Network error (CORS, offline, blocked) or aborted
  ...
  return (
    <section id="projects" className="relative mx-auto mt-24 max-w-7xl px-4">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 auto-rows-fr">
        ...
        {error && (
          <div className="col-span-full rounded-xl border border-destructive/20 bg-destructive/10 p-6 text-center">
          ...
          repos.map((p) => (
            <Animate key={p.id} className="relative">
              <Link
                ...
                className="block group relative overflow-hidden rounded-xl border border-white/10 bg-card/60 p-5 transition hover:bg-card/80 card-hover hover-glow hover-pop h-full flex flex-col justify-between"
                onMouseMove={(e) => {
